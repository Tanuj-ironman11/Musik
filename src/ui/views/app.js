...
//
// Renders into #main for the #/home route. Markup targets the class names
// from the original (pre-crash) components.css / layouts.css — .home-wrap,
// .home-topbar, .home-recent-grid, .home-playlists-row, .home-empty,
// .home-section, .home-playlist-card, etc. Re-add those old CSS files
// (not rewritten here per your call) and this should pick the styling
// back up as-is.
//
// Falls back to the empty state until library.getTracks()/getPlaylists()
// actually return data — no fake/placeholder tracks are rendered.

window.MusikViews = window.MusikViews || {};

// Shared click-motion trigger — mirrors player-bar.js's spawnQueuePop
// pattern (remove class, force reflow, re-add, clean up after animation
// ends). Exposed globally so library.js can reuse it rather than
// duplicating the same three lines everywhere a button needs a kick.
...
// Ticks the greeting/clock every second. Self-terminates once the
// elements are gone from the DOM (e.g. user navigated away from Home),
// so it never needs an explicit teardown hook from the router.
...
window.MusikViews.home = async function renderHome(main) {
  ...
  document.getElementById('home-add-music-btn').addEventListener('click', async (e) => {
    ...
    if (paths && paths.length && window.MusikPlayerUI) {
      ...
      renderHome(main); // refresh once tracks are added
    }
  });
...
function playlistCardHTML(playlist, allTracks) {
  ...
  // Pick up to 4 covers at random each render — re-shuffled every time
  // home renders, not stored/persisted.
  ...
  if (covers.length === 4) {
    ...
    // Fewer than 4 unique covers available — just use the first, full-bleed.
...
// Wires play/shuffle hover buttons + card-click navigation for any
// container holding .home-playlist-card elements (used on both Home's
// playlist row and the Library grid).
